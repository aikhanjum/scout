...
```
RedBoard (motors, 600 ms watchdog)  <-- USB serial -->  Pi 4 (RPLIDAR A2M8, camera, pose, map, audit, protocol server :8080)  <-- wifi -->  dashboard (Chrome)
```
...
| BRAIN, Pi service | `<name>` | `pi/` | Serial to the motor board, lidar, pose, occupancy map, clearance, wall following, camera labels, `/status` `/cmd` `/ws` `/map` `/photo` `/runs/latest` |
...
- `event`: kinds `obstacle`, `ramp` (both with `label`, `confidence`, `photo`), `width_pass|fail` (with `between`), `mark`, `run_start|stop`. Only width events are verdicts; obstacles are observations. The beep and the light went with the ESP32, so a verdict is spoken and shown only.
...
9. The Pi service must run with the lidar absent, the motor board absent, the camera absent, or all three. A missing device disables its own feature only and is logged loudly at startup. Serial ports are found by probing, never by `/dev/ttyUSB0`.
