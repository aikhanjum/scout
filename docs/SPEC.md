...
No odometry, no IMU, no cliff sensor: never near stairs or a drop. Position comes from fitting the room's rectangle, so it works in one rectangular room and stops working in a corridor, a cluttered room, or through a doorway — `pose` goes false and the map stops growing until it recovers. Parking close to something large enough to hide a wall loses the fit. RPLIDAR A1: about 12 m range, 5.5 Hz, poor on glass, mirrors and shiny black. Hobby gearmotors: slow, unmatched, smooth floors only, and four-wheel skid steer scrubs hard through turns. Pi 4 needs a real 5 V / 3 A source.
