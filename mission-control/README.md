...
- **The map** is the screen, drawn like a floor plan: the fitted room as a bold outline on white, every surface the lidar hit in ink, an obstacle seen all the way round as a solid block, the path Scout drove as a dashed line, Scout as the logo with an arc ahead of it for how far the way is clear (its ring turns gold and Scout holds still while an obstacle nearby is being confirmed, a second before the pin lands), and a marker for every obstacle, mark and width verdict. An obstacle marker is stamped "obstacle" and nothing more: Scout has no camera, so the lidar places it and that is all. Room dimensions sit in the corner; NO POSE and NO LIDAR appear there when they apply. Plain 2D canvas, no libraries.
...
- **Controls**, left to right in the order a run happens: the space name, START RUN, MARK, DOWNLOAD, CLEAR MAP.
  ...
  - **Stop run** also stops the motors, so a run never ends with Scout still driving, and closes the recording. Nothing is downloaded on its own.
  - **Download** is greyed out while a run is going, then hands out the run five ways. **Run report (.html)** is the one a person reads: one self contained page with the map drawn into it, the width verdicts, the obstacles and where they are, and what the numbers rest on. It opens in Chrome and prints to PDF. **Replay file (.ndjson)** is the recording just stopped (one map frame per 10 s, so replays fill the room in), kept until the next run starts. Behind them are the tables: events and telemetry (without the scans) as CSV, and the map as a table of cells (`x_mm,y_mm,state`) or as the PNG on screen. Everything since run start, or since connecting when no run was started.
...
- **Drive**: WASD or arrows, resent every 100 ms while held, `stop` on release. **Space is the E-STOP** and works anywhere on the page, which is why the pad has no stop button. Losing window focus sends `stop`. While the ring is gold Scout is holding still to confirm an obstacle within 650 mm and the keys are accepted but not applied; keep the key down and it carries on the moment the pin lands. Driving cancels wall following, which the Pi still accepts (`/cmd?c=roam`) but the dashboard no longer offers.
